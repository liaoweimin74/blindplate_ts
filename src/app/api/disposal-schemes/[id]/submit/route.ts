...
/** POST /api/disposal-schemes/[id]/submit 提交审核（DRAFT/REJECTED → PENDING_REVIEW），写审批留痕，需求 → DISPOSAL_PENDING_REVIEW */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const request = await db.workRequest.update({
      ...
      data: { status: 'DISPOSAL_PENDING_REVIEW' },
    })
    await pushNotifications({
      targetRoles: ['REVIEWER', 'MANAGER'],
      type: 'APPROVAL',
      title: '工艺处置方案待审核',
      content: `${request.code}（${request.title}）的处置方案 ${scheme.code} 已提交审核，请及时处理`,
      bizType: 'DISPOSAL',
      bizId: sid,
      bizCode: scheme.code,
      linkModule: 'approval-center',
    })
    // 审计留痕：处置方案提交审核，需求同步推进
    ...
    await logAudit({
      ...
      detail: auditFlowDetail(scheme.code, scheme.status, 'PENDING_REVIEW', `提交工艺处置方案审核，需求 ${request.code} 同步进入待审核`),
    })
    return NextResponse.json({ scheme: updated, request })
  } catch (e) {
    console.error('[POST /api/disposal-schemes/[id]/submit]', e)
    return jsonError(e instanceof Error ? e.message : '提交处置方案失败', 500)
  }
}
