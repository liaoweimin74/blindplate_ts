# 归档：盲板管理系统开发工作日志（Task 1 ~ 97-R-PUSH 历史全文）
> 本文件由 worklog.md 归档拆分生成，内容零删改；近期工作与需求基线请看 worklog.md（主文件）
...
Task ID: 85-qa
Agent: 主控(Z.ai Code)
Task: 用户提问「所有图元左上角绿色小圆点是什么」——调查并给出证据结论

Work Log:
- 定位代码：pid-config.tsx 3781-3790，`s.equipmentId != null` 时在图元左上角渲染 emerald 圆点（fill #059669 白描边 r4.5）+ `<title>`「已绑定设备：位号 名称」——Task 35【绑定可视化】设计特性，非 bug
- DB 实查（q_fk_tmp 脚本）：图20 15/15、图22 5/5、图23 5/5、图24 15/15、图26 2/2 全绑定 equipmentId；图27 仅 2/6（FV-101/FT-102/CV-201 仪表图元未绑→无绿点）——「所有图元都有绿点」实为「凡绑定设备主数据的图元都有」，主因是生成主数据/属性面板绑定已覆盖全部设备图元
- agent-browser 实测（admin 登录→台账→PID组态，图24）：画布 title「已绑定设备：…」15 条与 DB 吻合；决定性实验 elementFromPoint 在绿点中心命中网格 rect，绿点自身与父组 pointerEvents 均为 none
...
Task ID: 86
Agent: 主控(Z.ai Code)
Task: 用户两次反馈「还是只有四个点可以连线」——QA 复现、根因定位、双态命中区修复与全链回归

Work Log:
- 【实验定位（证据链）】①agent-browser 实测旧实现：12 用户单位 stroke 命中带在 100% 缩放（0.363 屏幕比例）下仅 ~4.4px 宽， practically 点不中，仅四边中点附近（有吸附反馈点提示处）可点——用户「只有四个点能连」体感成立；②挂起态点击目标图元本体：连线不建+挂起静默消失（handleCanvasClick 3124-3126 把非锚点点击一律当「空白取消挂起」吞掉）——第二击死锁实锤
- 【修复实现（pid-config.tsx，9 处）】①模块级 helper：anchorHitBandPath（evenodd 环带=包围盒外扩 18，本体留洞保选中/拖动）+ anchorHitSolidPath（contentInsetBox 实心全域）；②命中区常驻渲染（edit && !placing*），不再依赖 hover 先碰本体——鼠标从外围直接进入即亮框+吸附点；③双态切换：挂起时非源图元自动变实心（点任意位置即完成连线），源图元保持环带（换锚点/自环语义保留，点源本体/空白=取消）；④onAnchorClick 首击顺带 setSelected 选中源图元（属性面板立即可用）；⑤suppressShapeClickAtRef 拖动抑制：startDragShape onMove 置时间戳+400ms，onAnchorClick 开头消费检查——拖动图元释放的 click 不误判为连线点击；⑥缩放手柄移到锚点区之后渲染（顶层可点，选中态 resize 不被环带拦截）；⑦操作指引文案同步（「先点一个图元的边框环带，再点目标图元任意位置…点空白处取消」）
- 【关键 QA 插曲（agent-browser 工具坑，重要经验）】`agent-browser mouse down X Y` 的坐标参数会被当作 button 名静默失败——正确姿势是 `mouse move X Y` → `mouse down`（无参）→ `mouse up`；曾因该坑误判「真鼠标点击失败→代码有 bug」，后经 window 事件监听（注意：eval 返回值含逗号会被显示层吞成 {}，用 join(' | ') 拼接规避）证实事件序列 pointermove|mousedown|mouseup|click 完整到达且挂起成功——代码逻辑自始正确
- 【E2E 真鼠标全链回归（正确姿势，6/6 通过）】①点 E102 环带非中点(618,445)→挂起建立（常显点+14 实心目标实测）；②挂起态 DOM 验证：14 实心+1 环带（源）双态切换精确；③真鼠标点 E103 本体中心→连线创建（正交折线 229,460→229,406→595,406→595,352.6 完美曼哈顿路由）+挂起清零；④拖动 E103 移动 52px→conns 40→40 无误建+图元实移；⑤点空白→挂起 1→0 取消；⑥控制台零错误；实验数据全在本地 state 未保存，刷新即丢不污染 DB
- 验证：bun run lint 通过；tsc --noEmit src/ 0 错；0b2f91b..a885296 已随用户「先push」指令推送（代码部分），本条 worklog 随下轮提交
